      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="mb-14 text-center max-w-3xl mx-auto">
          ...
          <p className="text-base text-stone-600 mt-3 leading-relaxed">
            Dari landing page modern, permak file Excel berantakan, hingga aplikasi web custom.
          </p>
        </div>
        ...
        <div className="space-y-24">
          ...
          {/* ========================================================================= */}
          {/* ITEM 3: Aplikasi Presensi Custom & PWA (Projek Kaspul Hudur) */}
          {/* ========================================================================= */}
          <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-stone-200/80 pb-4">
              <div>
                <span className="text-xs font-bold text-purple-700 uppercase tracking-wider block mb-1">
                  03. Custom Web App &amp; PWA
                </span>
                <h3 className="text-2xl sm:text-3xl font-extrabold text-stone-900">
                  Aplikasi Presensi Custom (Kaspul Hudur)
                </h3>
              </div>
              <p className="text-xs sm:text-sm text-stone-600 max-w-md">
                Aplikasi absensi custom berbasis Next.js PWA &amp; Supabase SSR yang dikembangkan khusus sesuai kebutuhan operasional.
              </p>
            </div>

            {/* Technical Highlights Grid */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-5 rounded-xl bg-stone-900 text-stone-100 border border-stone-800 space-y-3 shadow-sm">
                <div className="flex items-center justify-between gap-2 text-xs font-bold text-[#42A5F5] uppercase tracking-wide font-mono">
                  <div className="flex items-center gap-1.5">
                    <Smartphone className="w-4 h-4" />
                    <span>Fitur Utama App Kaspul Hudur</span>
                  </div>
                  <span className="text-[10px] text-amber-400 bg-amber-950/80 px-2 py-0.5 rounded border border-amber-800/80 flex items-center gap-1">
                    <ShieldCheck className="w-3 h-3 text-amber-400" />
                    Private Repo
                  </span>
                </div>
                <ul className="text-xs text-stone-300 space-y-2 list-disc list-inside">
                  <li>Auth Google OAuth seamless via <code className="text-amber-300 font-mono">@supabase/ssr</code></li>
                  <li>Flat Session Feed mendukung presensi shift malam (lintas hari)</li>
                  <li>Sistem anti-spam double click &amp; auto pairing entry</li>
                  <li>Full PWA Installable langsung di HP (Android &amp; iOS)</li>
                </ul>
              </div>

              <div className="p-5 rounded-xl bg-[#1565C0]/5 border border-[#1565C0]/20 space-y-3 flex flex-col justify-between shadow-xs">
                <div>
                  <div className="flex items-center gap-2 text-xs font-bold text-[#1565C0] uppercase tracking-wide font-mono mb-3">
                    <Sparkles className="w-4 h-4" />
                    <span>Tech Stack &amp; Arsitektur</span>
                  </div>
                  <div className="flex flex-wrap gap-2 text-xs font-mono">
                    <span className="px-2.5 py-1 rounded bg-white text-stone-800 border border-stone-200 shadow-2xs">Next.js App Router</span>
                    <span className="px-2.5 py-1 rounded bg-white text-stone-800 border border-stone-200 shadow-2xs">Supabase SSR</span>
                    <span className="px-2.5 py-1 rounded bg-white text-stone-800 border border-stone-200 shadow-2xs">Zustand State</span>
                    <span className="px-2.5 py-1 rounded bg-white text-stone-800 border border-stone-200 shadow-2xs">Tailwind CSS</span>
                    <span className="px-2.5 py-1 rounded bg-white text-stone-800 border border-stone-200 shadow-2xs">PWA Manifest</span>
                  </div>
                </div>

                <div className="p-3 rounded-lg bg-white border border-stone-200 text-xs text-stone-600">
                  <span className="font-semibold text-stone-800 block mb-0.5">Verifikasi Repositori:</span>
                  Kode sumber tersimpan di private repository <code className="text-[#1565C0] font-mono font-bold">Deearss/kaspul-hudur</code>.
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-end">
              <a
                href="#kontak"
                className="inline-flex items-center gap-1.5 px-5 py-2.5 rounded-lg bg-[#1976D2] hover:bg-[#1565C0] text-white text-xs font-bold active:scale-95 transition-all shadow-sm"
              >
                <span>Diskusi Aplikasi Custom</span>
                <ArrowUpRight className="w-4 h-4" />
              </a>
            </div>
          </div>
